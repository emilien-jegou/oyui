...
impl<'a> GutterSign<'a> {
    pub fn compute_base_style(&self) -> Style {
        ...
        if self.is_add {
            let mut col = self.theme.add_fg.clone();
            if !self.is_staged {
            ...
            prefix_style = prefix_style.fg(col.into());
        } else if self.is_del {
    ...
    pub fn render(&self) -> Cell<'a> {
        ...
        if !bg_calc.char_by_char() {
            ...
            let final_style = prefix_style.bg(bg.into());
        ...
        for (i, c) in prefix.chars().enumerate() {
            ...
            spans.push(Span::styled(c.to_string(), prefix_style.bg(bg.into())));
        }
